...
export default function GenresSelector({ badge = 0, onClick }) {
  const renderBadge = () => {
    if (!badge) return null;
    return (
      <span
        className="dark:bg-dark-subtle bg-light-subtle text-white absolute top-0 right-0 translate-x-2 text-xs -translate-y-2 w-5 h-5 rounded-full flex justify-center items-center
      "
      >
        {badge <= 9 ? badge : "9+"}
      </span>
